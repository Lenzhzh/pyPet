...
class Deskpet(QWidget):
    ...
    def init_optional_settings(self):
        ...
        if self.setting_manager.get('click audio player') and self.setting_manager.get('click audio player') != "NONE":
            ...
            logger.info("Click audio enable !!!")
        if self.setting_manager.get('visible console'):
            logger.show()
            logger.info("Logger visbled !!!")
    ...
    def close(self):
        super().close()
        if not(logger.Console.isVisible()) :
            sys.exit()
    ...
    def switch_state(self, state: PetState):
        ...
        logger.info(f"pet state shift to {state.name} !!!")
    ...
    def _check_short_click(self):
        ...
        if ((now_time - self._last_left_click)*1000 < 120) :
            logger.info("A short click listened !!!")
            return True
        else :
            logger.info("A long click listened !!!")
            return False       
    ...
    def start_random_move(self):
        if self.current_state == PetState.STANDBY:
            ...
            logger.info(f"A random move start !")
    ...
    def debug(self):
        ...
        debug每秒钟输出一次的内容
        '''
        logger.debug("It's a console test !")
